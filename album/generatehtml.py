...
def create_html(root, files, source, target):
    file_paths = path_generator(root, files)
    jinja2_template_string = open(source, 'r', encoding="utf8").read()  # Reads the sample html file
    ...
    htmlfilepath = os.path.join(root, target)
